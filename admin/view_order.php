...
<th>Nama</th><th>No IC</th><th>Kod Tempahan</th><th>Tarikh Terima</th><th>Tarikh Ambil</th><th>Status</th><th>Catatan</th><th>Jenis Pakaian</th><th>Gambar Kain</th>

<?php 
    
    $order = "SELECT * FROM orders WHERE status='Dalam Progress'";
    $i=1;
...
window.onclick = function(event) {
  if (event.target == modal) {
    ...
    <td>
	<button class="w3-button w3-indigo w3-hover-light-blue"><a style="text-decoration:none" href="update_notice.php?id_orders=<?php echo $id_orders ?>" onclick="window.open('send_notis.php?id_orders=<?php echo $id_orders?>');return true;">Hantar</a></button>
	</td>
    <td ><?php 
        
        if($notis == "Belum"){
            ?> <font style="font-weight:bold" color="red"><?php echo $notis ?></font> <?php
        } else { ?>
            <font style="font-weight:bold" color="green"><?php echo $notis ?></font> <?php
        }
